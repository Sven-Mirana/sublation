...
if [[ "${CONFIRM_PUBLISH:-}" != "1" ]]; then
  ...
  echo "Review RELEASE-v4.0.md and checksums.sha256 first."
...
if command -v gh >/dev/null 2>&1; then
  gh release create "$VERSION" --title "Skill Sublation $VERSION" --notes-file RELEASE-v4.0.md
else
  echo "gh CLI not found; create the GitHub release manually using RELEASE-v4.0.md."
fi
